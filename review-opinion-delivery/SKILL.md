...
4. Build the v1.2 `delivery_manifest.json` with immutable `item_id`, display `item_no`, approved fields, provenance, evidence strategy, and zero or more `evidence_images`. Distinguish reviewer-approved regulation `无` from an unreviewed placeholder in the verification record. Fill `verification_log.csv` and `edit_log.csv`.
...
- One core issue normally forms one opinion. Put synchronized plan/elevation/detail requirements in the main opinion unless the reviewer explicitly separates them.
- Use drawing number and drawing name in the Word body; keep PDF page, mark number, and crop coordinates in the evidence chain and logs.
