...
For v1.4 also record `rule_id`, `atomic_check_id`, `decision_state`, `discovery_track`, `applicability_basis`, `comparison_method`, `comparison_record`, `calculation_record`, `open_reason`, and `reviewer_gate`. Read `review-rule-schema.md` before editing generated rows.
...
For v1.4, `符合` and `不符合` additionally require a rule ID, an applicability basis, actual facts, comparison record, any required calculation record, `decision_state=resolved`, and `reviewer_gate=已复核`. Missing any one of them requires `needs_review/需核验`.
...
- A v1.4 formal issue may come only from a resolved `不符合` atomic check whose `issue_id` matches the candidate.
